    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.banner.file_path) {
          setDesktopPreview(
            data.banner.file_path.startsWith("http")
              ? data.banner.file_path
              : `/uploads/carousel/${data.banner.file_path}`
          );
        }
        ...
        toast.error("Offer banner not found");
      }
    } catch (err) {
      console.error(err);
      toast.error("Error fetching offer banner");
    }
    setLoading(false);
  };
  ...
  return (
    <div className="p-6">
      <div className="bg-white shadow-sm border border-gray-100 rounded-2xl p-8 max-w-xl mx-auto">
        {/* HEADER */}
        <h1 className="text-2xl font-bold text-[#1a3a6b] mb-1">Edit Offer Banner</h1>
        <p className="text-sm text-gray-400 mb-7">Update offer banner details below</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 text-black">
          ...
          <div>
            ...
            <label className="flex items-center w-full border border-gray-300 rounded-xl overflow-hidden cursor-pointer hover:border-gray-400 focus-within:ring-2 focus-within:ring-blue-500 transition-colors bg-white">
              <span className="px-4 py-2.5 bg-gray-100 border-r border-gray-300 text-sm font-semibold text-gray-800 whitespace-nowrap hover:bg-gray-200 transition-colors">
                Choose File
              </span>
              <span className="px-3 py-2.5 text-sm text-gray-400 truncate">
                {desktopFile ? desktopFile.name : "No file chosen"}
              </span>
          ...
          <div>
            ...
            <label className="flex items-center w-full border border-gray-300 rounded-xl overflow-hidden cursor-pointer hover:border-gray-400 focus-within:ring-2 focus-within:ring-blue-500 transition-colors bg-white">
              ...
              <span className="px-3 py-2.5 text-sm text-gray-400 truncate">
                {mobileFile ? mobileFile.name : "No file chosen"}
              </span>
          ...
          <div className="flex items-center justify-between pt-3">
            <button
              type="submit"
              disabled={submitting}
              className={`bg-blue-600 text-white px-8 py-2.5 rounded-xl text-sm font-semibold hover:bg-blue-700 transition-colors shadow-sm ${
            ...
            <button
              type="button"
              onClick={() => router.push("/admin/offer-banners")}
