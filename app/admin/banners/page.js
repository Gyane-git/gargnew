...
export default function BannerList() {
  ...
  const filteredBanners = banners.filter((banner) => banner.product_code?.toLowerCase().includes(search.toLowerCase()));

  const totalEntries = filteredBanners.length;
  const totalPages = Math.max(1, Math.ceil(totalEntries / entriesPerPage));

  const paginatedBanners = filteredBanners.slice((currentPage - 1) * entriesPerPage, currentPage * entriesPerPage);
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1 text-sm">Manage your banner collection</p>
      </div>
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm mx-4 text-center">
            ...
            <h3 className="text-lg font-bold text-gray-800 mb-1">Delete Banner?</h3>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={deleteBanner} className="flex-1 py-2.5 rounded-xl bg-red-500 text-white text-sm font-semibold hover:bg-red-600 transition-colors">
                Delete
              </button>
            </div>
          </div>
