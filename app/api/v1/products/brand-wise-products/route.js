...
//     // ---- Step 1: total count for this brand ----
//     const [[totalRow]] = await pool.query(`SELECT COUNT(*) AS total FROM products p WHERE p.brand_id = ? ${statusClause}`, [brand_id]);

//     // ---- Step 2: paginated product ids (no fan-out joins here) ----
...
//     // ---- Step 3: full joined data for just those product ids ----
//     const [rows] = await pool.query(
//       `SELECT
//         p.id AS product_id,
//         p.product_code,
...
//     // Preserve the paginated order (p.id DESC) from Step 2
...
//     });
//   } catch (error) {
//     console.error("brand-wise-products error:", error);
