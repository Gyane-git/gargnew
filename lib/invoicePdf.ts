...
function buildInfoRows(order: any): {
  leftRows: Array<{ label: string; value?: string }>;
  rightRows: Array<{ label: string; value?: string; badge?: { text: string; status: string }; extraBadge?: { text: string; status: string } }>;
  orderStatus: string;
} {
  ...
  return { leftRows, rightRows, orderStatus };
}
...
function drawInfoSection(b: PageBudget, pages: PdfPage[], order: any): PageBudget {
  ...
  const { leftRows, rightRows, orderStatus } = buildInfoRows(order);
...
function drawItemsTableHeader(b: PageBudget, cols: { colNoX: number; colDescX: number; colQtyRightX: number; colPriceRightX: number; colAmountRightX: number }): number {
  ...
  b.page.text(cols.colNoX, textY, "NO.", { size: 9.5, bold: true, color: COLOR_WHITE });
  b.page.text(cols.colDescX, textY, "PRODUCT DESCRIPTION", { size: 9.5, bold: true, color: COLOR_WHITE });
  b.page.text(cols.colQtyRightX, textY, "QTY", { size: 9.5, bold: true, color: COLOR_WHITE, align: "right" });
  b.page.text(cols.colPriceRightX, textY, "UNIT PRICE", { size: 9.5, bold: true, color: COLOR_WHITE, align: "right" });
  b.page.text(cols.colAmountRightX, textY, "AMOUNT", { size: 9.5, bold: true, color: COLOR_WHITE, align: "right" });
  return barTop - headerHeight;
}
...
function drawItemsSection(b: PageBudget, pages: PdfPage[], order: any): PageBudget {
  ...
  const cols = {
    colNoX: PDF_MARGIN + 6,
    colDescX: PDF_MARGIN + 32,
    colQtyRightX: PDF_MARGIN + 320,
    colPriceRightX: PDF_MARGIN + 410,
    colAmountRightX: PDF_CONTENT_RIGHT - 6,
  };

  const items = Array.isArray(order.orderItems) ? order.orderItems : Array.isArray(order.items) ? order.items : [];

  let tableTop = b.y;
  b.y = drawItemsTableHeader(b, cols);
  ...
  items.forEach((item: any, index: number) => {
    ...
    const sku = product.sku ?? item.sku ?? null;
    const quantity = Number(item.quantity ?? 0);
    const actualPrice = Number(item.actual_price ?? 0);
    const subtotal = Number(item.subtotal_without_tax ?? 0);

    const nameLines = wrapPdfText(productName, 46);
    const skuLines = sku ? wrapPdfText(`SKU: ${sku}`, 46) : [];
    const rowLines = nameLines.length + skuLines.length;
    const rowHeight = rowPaddingY * 2 + rowLines * lineHeight;

    // If a new page is needed mid-table, close the current box and start a fresh header on the next page.
    ...
    const rowTop = b.y;
    let textY = rowTop - rowPaddingY - 8;

    b.page.text(cols.colNoX, textY, String(index + 1), { size: 9.5 });
    b.page.text(cols.colDescX, textY, nameLines[0] ?? "", { size: 9.5, bold: true });
    for (let i = 1; i < nameLines.length; i += 1) {
      textY -= lineHeight;
      b.page.text(cols.colDescX, textY, nameLines[i], { size: 9.5, bold: true });
    }
    skuLines.forEach((line) => {
      textY -= lineHeight;
      b.page.text(cols.colDescX, textY, line, { size: 8.5, color: COLOR_GRAY_666 });
    });

    const firstLineY = rowTop - rowPaddingY - 8;
    b.page.text(cols.colQtyRightX, firstLineY, String(quantity), { size: 9.5, align: "right" });
    b.page.text(cols.colPriceRightX, firstLineY, `Rs. ${formatNumber(actualPrice)}`, { size: 9.5, bold: true, align: "right" });
    b.page.text(cols.colAmountRightX, firstLineY, `Rs. ${formatNumber(subtotal)}`, { size: 9.5, bold: true, align: "right" });

    b.y = rowTop - rowHeight;

    // row separator (border-bottom: 1px solid #ccc), skip after the last item
  ...
  finishBox(tableTop);
  b.y -= 20; // items-section margin-bottom equivalent
