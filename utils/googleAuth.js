...
export const handleGoogleLogin = async ({ token, unique_id } = {}) => {
  const normalizedToken = String(token || "").trim();
  const providedUniqueId = String(unique_id || "").trim();

  if (!normalizedToken) {
    return { success: false, errors: [{ message: "Google token is required." }], status: 400 };
  }

  const googleUser = await getGoogleProfile(normalizedToken);
  ...
  if (!email) {
    return { success: false, errors: [{ message: "Google account email not found." }], status: 422 };
  }
  ...
  const now = new Date();
  const hashedPassword = await bcrypt.hash(`${normalizedToken}:${email}:${Date.now()}`, 10);
  ...
  if (existingRows.length > 0) {
    ...
    await pool.query(`UPDATE users SET ${setParts.join(", ")} WHERE id = ?`, [...values, userId]);
  } else {
    ...
    const [result] = await pool.query(`INSERT INTO users (${fields.join(", ")}) VALUES (${placeholders})`, values);
  ...
  return {
    ...
    user: {
      ...
      image: user.profile_photo_path || picture || null,
    },
  };
};
